...
const handler: Handler = (req, res, next) => {
  ...
  res.on("close", () => {
    ...
    console.log(
      `${methodWithColor(req.method)}\t${statusCodeWithColor(
        res.statusCode
      )}\t${req.originalUrl} ${chalk.gray(`took ${timeWithColor(timeTaken)}`)}`
    );
  });
  next();
};

function methodWithColor(method: string) {
  switch (method) {
    case "GET":
...
function statusCodeWithColor(statusCode: number) {
...
function timeWithColor(time: number) {
  if (time < 100) {
